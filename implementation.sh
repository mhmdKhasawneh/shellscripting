...
declare -A file_groups
  ...
  while IFS= read -r -d '' file; do
    ...
    file_info="$filename|$size|$permissions|$last_modified"
  ...
  for owner in "${!file_groups[@]}"; do
    total_size=0
    while IFS="|" read -r _ size _ _; do
      total_size=$((total_size + size))
    done <<< "${file_groups["$owner"]}"
    sorted_groups+=("$owner|$total_size")
  done

  IFS=$'\n' sorted_groups=($(sort -t '|' -k2,2n <<<"${sorted_groups[*]}"))
  ...
  for group in "${sorted_groups[@]}"; do
    ...
    while IFS=$'\n' read -r file_info; do
      IFS="|" read -r filename size permissions last_modified <<< "$file_info"
